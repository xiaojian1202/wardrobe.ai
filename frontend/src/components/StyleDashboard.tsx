import { Sparkles, Palette, Layers, PieChart, type LucideIcon } from 'lucide-react';
import { motion } from 'framer-motion';
import type { WardrobeStats } from '../types';
 ...
 */
export default function StyleDashboard({ stats, isLoading = false }: StyleDashboardProps) {
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-soft-bg shadow-extruded rounded-[2rem] p-6 flex items-center gap-5 animate-pulse"
          >
            <div className="h-12 w-12 rounded-2xl bg-soft-bg shadow-inset-sm shrink-0" />
            <div className="space-y-2 flex-1">
              <div className="h-2 w-16 bg-soft-muted/20 rounded" />
              <div className="h-4 w-24 bg-soft-muted/30 rounded" />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <section aria-label="Wardrobe style statistics" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
      {cards.map((card, index) => (
        ...
        >
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-soft-bg shadow-inset-sm shrink-0">
            <card.icon size={20} className={card.color} strokeWidth={2.5} aria-hidden="true" />
          </div>
